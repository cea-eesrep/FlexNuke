...
from .cpp_credit_K_interprocessing import credit_K_interprocessing_cpp
...
def credit_K_interprocessing(results:pd.DataFrame, 
    ...
    for flexible_unit in flexible_units:
        ...
        new_results[name+"_creditELPO"] = credit_K_interprocessing_cpp(mode, p_max, efficiency, fuel_weight, list(new_results[name+"_creditELPO"]), list(new_results[name+"_is_step_ELPO"]), list(new_results[name+"_PO"]), list(new_results[name+"_electricity"]), list(new_results[name+"_fpd"]), dict_K0, dict_A_i, dict_B_j)
    # end for units
