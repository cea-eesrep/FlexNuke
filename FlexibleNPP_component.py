...
def credit_K_interprocessing(results:pd.DataFrame, flexible_units, global_time_shift, global_future_size, dict_K0, dict_A_i, dict_B_j):
    ...
    for flexible_unit in flexible_units:
        ...
        #     # end if
        #     # print(f"Ai*24: {A_i}  ---  Bj*24: {B_j}")

        #     new_creditK = past_past_step_creditELPO - A_i / 24 + B_j / 24
        #     # print(f"New crédit for step {past_step_index}: {new_creditK}")
        #     # print(f"Old crédit for step {past_step_index}: {results.loc[past_step_index, name+'_creditELPO']}")
            
        #     new_results.loc[past_step_index, name+"_creditELPO"] = new_creditK
        # # end for timesteps

        new_results[name+"_creditELPO"] = post_processor.credit_K_interprocessing_cpp(mode, p_max, efficiency, fuel_weight, list(new_results[name+"_creditELPO"]), list(new_results[name+"_is_step_ELPO"]), list(new_results[name+"_PO"]), list(new_results[name+"_electricity"]), list(new_results[name+"_fpd"]), dict_K0, dict_A_i, dict_B_j)
    # end for units

    # print(new_results.columns)
    stop_time = time.time()
    print(f"Post-processing duration : {stop_time - start_time}")
    return new_results